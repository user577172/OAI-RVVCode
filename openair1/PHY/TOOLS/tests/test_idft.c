...
#define FOREACH_IDFTSZ_working(SZ_DEF) \
  SZ_DEF(64)                            \
  SZ_DEF(128)                           \
  SZ_DEF(256)                           \
  SZ_DEF(512)                           \
...
struct {
  int size;
} const idftFtab[] = {FOREACH_IDFTSZ_working(SZ_iPTR)};
...
int main(void)
{
  int ret = 0;
  load_dftslib();
  c16_t *d16 = malloc16(idftFtab[sizeofArray(idftFtab) - 1].size * sizeof(*d16));
  c16_t *o16 = malloc16(idftFtab[sizeofArray(idftFtab) - 1].size * sizeof(*d16));
  for (int sz = 0; sz < sizeofArray(idftFtab); sz++) {
    const int n = idftFtab[sz].size;
    cd_t data[n];
    ...
    for (int coeff = 0; coeff < sizeofArray(coeffs); coeff++) {
      ...
      idft(get_idft(n), (int16_t *)d16, (int16_t *)o16, 1);
      if (n == 12) {
        for (int i = 0; i < n; i++) {
    ...
    printf("done IDFT size %d (evm (%%), avg samples amplitude) = ", n);
    for (int coeff = 0; coeff < sizeofArray(coeffs); coeff++)
    ...
    if (i == sizeofArray(coeffs)) {
      printf("IDFT size: %d, minimum error is more than 1%%, setting the test as failed\n", n);
      ret = 1;
    }
    fflush(stdout);
  }
